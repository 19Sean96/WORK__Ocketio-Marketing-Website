...
export default function BlogCategory({ blog_posts, params }) {
  ...
  useEffect(() => {
    ...
    if (activeCategory === 'latest') {
      setFilteredBlogList(blogs.filter(blog => {
        const now = new Date()
      ...
      setFilteredBlogList(blogs.filter(blog => blog.all_categories.includes(activeCategory)))
    }
...
export async function getStaticPaths() {
  ...
  const paths = result.data.data.map(category => ({ params: { category: category.id } }))
  return {
      paths,
      fallback: false
  }
